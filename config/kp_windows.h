#define WN_PND kp HASH
#define WN_AT  kp DQT
#define WN_DQT kp AT
#define WN_HSH kp NON_US_HASH
#define WN_PPE kp PIPE2
#define WN_TLD kp TILDE2 
#define WN_PWD kp LC(LEFT)
#define WN_NWD kp LC(RIGHT)
#define WN_ZIN kp LC(PLUS)
#define WN_ZOT kp LC(MINUS)
#define WN_PSC kp PSCRN     
#define WN_CUT kp LC(X)
#define WN_CPY kp LC(C)
#define WN_PST hm LSHIFT LC(V)
#define WN_UND kp LC(Z)

#define WN_A hm LGUI A
#define WN_S hm LALT S
#define WN_D hm LCTRL D
#define WN_F hm LSHIFT F
#define WN_J hm LSHIFT J
#define WN_K hm LCTRL K
#define WN_L hm LALT L
#define WN_SEMI hm LGUI SEMI

#define WN_WNL hm LALT LG(LEFT)
#define WN_WNU hm LCTRL LG(UP)
#define WN_WNR hm LSHIFT LG(RIGHT)